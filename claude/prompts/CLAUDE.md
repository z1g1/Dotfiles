...
> **Source of truth: `~/.claude/settings.json`.** The list below is documentation only — Claude Code enforces permissions from `settings.json` / `settings.local.json`, not from CLAUDE.md. Previously this section was not being enforced; the entries have been migrated into `~/.claude/settings.json` as `Bash(foo:*)` rules. When adding or removing entries, update **both** the settings file (to change enforcement) and the list below (to keep docs in sync).
